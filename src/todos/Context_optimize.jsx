...
NewTodoContext.displayName = "NewTodoContext";

function useNewTodoContext() {
	return useContext(NewTodoContext);
}

const TodoFilterContext = createContext();
TodoFilterContext.displayName = "TodoFilterContext";

function useTodoFilterContext() {
...
TodoActionContext.displayName = "TodoActionContext";

function useTodoStateContext() {
	return useContext(TodoStateContext);
}

function useTodoActionContext() {
	return useContext(TodoActionContext);
}

function useTodoContext() {
	return { ...useTodoStateContext(), ...useTodoActionContext() };
}
...
function TodoProvider({ children }) {
	...
	const action = useMemo(
		...
		[clearTodos, toggleDone, addTodo]
	);

	const newTodoValue = useMemo(() => ({ newTodo, changeNewTodo, error }), [newTodo, changeNewTodo, error]);
	const filterValue = useMemo(() => ({ filter, changeFilter }), [filter, changeFilter]);

	return (
		<TodoStateContext.Provider value={state}>
			<TodoActionContext.Provider value={action}>
				<NewTodoContext.Provider value={newTodoValue}>
					<TodoFilterContext.Provider value={filterValue}>{children}</TodoFilterContext.Provider>
...
const ClearButton = memo(function ClearButton() {
	const { clearTodos: onClick } = useTodoActionContext();
...
const RemainingItems = memo(function RemainingItems() {
	...
	const { filter } = useTodoFilterContext();
...
const Filters = memo(function Filters() {
	const { filter: value, changeFilter: onChange } = useTodoFilterContext();
...
const NewTodo = memo(function NewTodo() {
	const { newTodo: value, changeNewTodo: onChange } = useNewTodoContext();
	const { addTodo: onSubmit } = useTodoActionContext();
...
const Error = memo(function Error() {
	const { error: message } = useTodoStateContext();
